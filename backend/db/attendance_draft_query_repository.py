...
class PostgresAttendanceDraftQueryRepository:
    ...
    def get_lesson_detail(self, lesson_id: int) -> DraftLessonView:
        ...
        return DraftLessonView(
            id=int(lesson_row[0]),
            import_batch_id=int(lesson_row[1]),
            course_name=str(lesson_row[2]),
            lesson_date=lesson_row[3].isoformat(),
            source_meeting_id=str(lesson_row[4]),
            status=str(lesson_row[5]),
            is_ignored=bool(lesson_row[6]),
            threshold_ratio=float(lesson_row[7]),
            meeting_start_at=_ensure_datetime(lesson_row[8]).isoformat(),
            meeting_end_at=_ensure_datetime(lesson_row[9]).isoformat(),
            effective_start_at=_ensure_datetime(lesson_row[10]).isoformat(),
            break_point_at=_optional_datetime_iso(lesson_row[11]),
            effective_end_at=_ensure_datetime(lesson_row[12]).isoformat(),
            break_source=str(lesson_row[13]),
            effective_start_source=str(lesson_row[14]),
            effective_end_source=str(lesson_row[15]),
            warnings=list(lesson_row[16] or []),
            diagnostics=dict(lesson_row[17] or {}),
            summary=summary,
